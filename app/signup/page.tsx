  ...
  if (success) {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6">
        ...
        <p className="text-center text-muted-foreground text-sm mb-4 max-w-[280px]">
          We&apos;ve sent you a confirmation link. Click it to activate your account.
        </p>
        <div className="bg-warning/10 border border-warning/20 rounded-xl px-4 py-3 mb-6 max-w-[320px]">
          <p className="text-xs text-warning font-medium text-center">
            <MailCheck className="inline h-3.5 w-3.5 mr-1" /> Don&apos;t see it? Check your <span className="font-bold">Spam / Junk</span> folder!
          </p>
        </div>
      ...
      <div className="pt-8">
        ...
        <div className={`transition-opacity duration-500 ${reveal ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <p className="text-muted-foreground text-sm">
            Start managing group expenses effortlessly
          </p>
        </div>
      </div>

      {/* Form */}
      <form onSubmit={handleSignup} className={`flex flex-col gap-4 mt-8 transition-opacity duration-500 ${reveal ? 'opacity-100' : 'opacity-0'}`}>
        {error && (
